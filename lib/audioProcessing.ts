  ...
  try {
    ...
    const commonPaths = {
      ...
      linux: {
        ...
        ffprobe: ['/usr/bin/ffprobe', '/usr/local/bin/ffprobe', '/snap/bin/ffprobe']
      },
      win32: {
...
export function buildMasteringChain(settings: MasteringSettings, analysis: AudioAnalysis): string[] {
  ...
  console.log('🎛️ Building mastering chain with settings:', settings);
...
export async function cleanupTempFiles(tempDir: string, maxAge: number = 24 * 60 * 60 * 1000): Promise<void> {
  try {
    const tempRoot = path.join(process.cwd(), tempDir);
